export default class Search {
  ...
  static async makeApiCall(input) {
    try {
      const url = `https://sandbox.iexapis.com/stable/search/${input}?token=${process.env.API_KEY2}`;//`https://sandbox.iexapis.com/stable/search/${input}?token=${process.env.API_KEY}`;
