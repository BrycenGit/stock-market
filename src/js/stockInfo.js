export default class StockInfo {
  ...
  static async getData(symbol) {
    let urlCompany = `https://sandbox.iexapis.com/stable/stock/${symbol}/company?token=${process.env.API_KEY2}`;
    const responseCompany = await StockInfo.makeApiCall(urlCompany);
    let urlLogo = `https://sandbox.iexapis.com/stable/stock/${symbol}/logo?token=${process.env.API_KEY2}`;
    const responseLogo = await StockInfo.makeApiCall(urlLogo);
    let urlQuote = `https://sandbox.iexapis.com/stable/stock/${symbol}/quote?token=${process.env.API_KEY2}`;
